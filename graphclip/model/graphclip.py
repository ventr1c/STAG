...
class GraphCLIP(torch.nn.Module):
    def __init__(self, graph_input_dim, graph_hid_dim, graph_num_layer, attn_kwargs, text_model='tiny', mode='train'):
        super().__init__()
        if mode == 'train':
            ...
            self.graph_model = MyGPS(in_dim=graph_input_dim, channels=graph_hid_dim, out_dim=graph_hid_dim,
                               pe_dim=8, num_layers=graph_num_layer, attn_type='multihead', attn_kwargs=attn_kwargs)
    ...
    def encode_graph(self, batch):
        if hasattr(batch, "weights"):
            graph_embs, center_embs = self.graph_model(batch.x, batch.pe, batch.edge_index, batch.batch,
                                                       batch.root_n_index, batch.weights)
        else:
    ...
    def encode_text_emb(self, input_embeds, token_type_ids, attention_mask):
