...
def set_module_parameter(module, param_name, new_param):
    """
    根据参数名称（如 'linear.weight'）找到并替换模块中的参数。
    """
    ...
    # 使用 setattr 替换参数，这会自动处理好参数的注册
    setattr(sub_module, attrs[-1], new_param)
...
def load_model_pretrained(model_name, pretrain_model_path): 
    ...
    state_dict = torch.load(pkl_files[0], map_location=device)


    # 4. 遍历模型的 meta 参数，并用具体的、带有数据的参数进行替换
    # 使用 list(model.named_parameters()) 来创建一个副本，因为循环中会修改模型结构
    # for name, _ in list(model.named_parameters()):
    #     # 从 state_dict 中获取数据，并创建一个新的、位于目标设备上的 nn.Parameter
    #     new_param = nn.Parameter(state_dict[name])
    #
    #     # 调用帮助函数，将模型中的 meta 参数替换为新的具体参数
    #     set_module_parameter(model, name, new_param)
    if hasattr(model, 'to_empty'):
        model.to_empty(device=device)
    # model.to(device)  # 确保整个模型移动到设备
    # pt_files = glob.glob(osp.join(pretrain_model_path, '*.pt'))
...
def transfer_param_tograph(clip_graph, gnn):
    ...
    gnn.to_empty(device="cuda:0")
    gnn.load_state_dict(gnn_state_dict)
    return gnn

def duplicate_row_at_index(a, index):
    """
    在 PyTorch 张量 a 的指定 index 下方复制该行。

    Args:
        a (torch.Tensor): 输入的 2D 张量，维度为 (m, n)。
        index (int): 要复制的行的索引。

    Returns:
        torch.Tensor: 增加了新行的张量，维度为 (m+1, n)。
    """
    # 检查 index 是否有效
    m = a.size(0)
    if not 0 <= index < m:
        raise IndexError(f"Index {index} is out of bounds for tensor with {m} rows.")

    # 1. 选取从开头到 index（包含）的所有行
    top_part = a[:index + 1]

    # 2. 选取要被复制的那一行 (使用切片 a[index:index+1] 来保持其二维性)
    row_to_duplicate = a[index:index + 1]

    # 3. 选取从 index+1 到末尾的所有行
    bottom_part = a[index + 1:]

    # 4. 将这三部分按顺序垂直拼接起来
    new_a = torch.cat((top_part, row_to_duplicate, bottom_part), dim=0)
    ...
    def forward(
        ...
        if graph_tower is not None and (input_ids.shape[1] != 1 or self.training) and graph_data is not None:
                ...
                if type(graph_data) is list:
                    ...
                    for g in graph_data:
                        if type(g) is Data:
                            ...
                            # node_forward_out = g.target_embeddings
                            if g.target_node != 0:
                                # 直接利用 Python 的元组解包特性进行交换
                                node_forward_out[0], node_forward_out[g.target_node] = \
                                    node_forward_out[g.target_node].clone(), node_forward_out[0].clone()
                            ...
                            # input_ids = duplicate_row_at_index(input_ids, my_count)
                            # inputs_embeds = duplicate_row_at_index(inputs_embeds, my_count)
                            my_count += 2
                else:
            ...
            if type(graph_data) is list:
                ...
                raise ValueError(f'graph_node_reps is expected to be a list but got {type(graph_data)}')
            dummy_graph_features = torch.zeros(256, 384, device=inputs_embeds.device, dtype=inputs_embeds.dtype)
            ...
            for cur_input_ids, cur_input_embeds in zip(input_ids, inputs_embeds):
                ...
                if graph_tower.config.use_graph_start_end:
                    ...
                    for graph_start_token_pos in graph_start_tokens:
                        ...
                        # cur_graph_features = cur_graph_features / cur_graph_features.norm(dim=-1,
                        #                                                                   keepdim=True)
