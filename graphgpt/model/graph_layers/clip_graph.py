...
class CLIP(nn.Module):
    def __init__(self,
        ...
        # )
        self.transformer = AutoModel.from_pretrained('/root/lanyun-tmp/GraphCLIP/all-MiniLM-L6-v2')
        # model_path = '/root/autodl-tmp/GraphCLIP/all-MiniLM-L6-v2'
        # # 1. 只加载模型的配置信息
        # config = AutoConfig.from_pretrained(model_path)
        #
        # # 2. 使用 init_empty_weights() 上下文，显式地在 meta device 上创建模型的“空壳”
        # #    这正是您的环境在后台隐式做的事情，我们现在把它明确化。
        # with init_empty_weights():
        #     self.transformer = AutoModel.from_config(config)
        #
        # # 此时 self.transformer 是一个没有权重的 meta 对象，不能移动
        # self.transformer.tie_weights()  # 对于某些模型，在加载权重前需要这一步
        #
        # # 3. 使用 accelerate 的核心函数来加载权重并将其直接放置到目标设备上
        # #    这个函数就是为了解决 "meta tensor" 问题而设计的。
        # #    它会将权重从 `model_path` 加载进 `self.transformer` 的空壳中，
        # #    并智能地将最终的模型放在 `device` 上。
        # self.transformer = load_checkpoint_and_dispatch(
        #     self.transformer,
        #     model_path,
        #     device_map={'': "cuda:0"},  # 关键参数：将整个模型映射到指定设备
        #     no_split_module_classes=[]  # 对于小模型，防止被不必要地分割
        # )
    ...
    def encode_image(self, g):
        embs = self.gnn(g)
        # idx_train = idx_train.to(embs.device)
        # idx_train = idx_train
        embs = embs[g.root_n_index]
        return embs
    ...
    def encode_text(self, input_ids, attention_mask=None):
        ...
        if attention_mask is None:
            attention_mask = torch.ones(input_ids.shape[1]).unsqueeze(0).cuda()
    ...
    def forward(self, g, s_n, t_n, s_n_text, t_n_text,device, training=True):

        s_image_features = self.encode_graph(s_n, g)
        s_n_text_inputs = s_n_text.input_ids.to(device)
        s_n_text_attention_mask = s_n_text.attention_mask.to(device)
        s_text_features = self.encode_text(s_n_text_inputs, s_n_text_attention_mask)

        t_n_text_inputs = t_n_text.input_ids.to(device)
        t_n_text_attention_mask = t_n_text.attention_mask.to(device)
        t_text_features = self.encode_text(t_n_text_inputs, t_n_text_attention_mask)
