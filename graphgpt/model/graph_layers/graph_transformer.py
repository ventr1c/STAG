...
class graph_transformer(nn.Module):
    ...
    def forward(self, g):
        ...
        # x = g.graph_node
        x = g.x
        if hasattr(g, "weights"):
            mask = g.weights == 1
            selected_indices = t.where(mask)[0]
            g.edge_index = g.edge_index[:, selected_indices]
        x, self.W_P.weight.data, self.W_P.bias.data = Mv2Samedevice([x, self.W_P.weight, self.W_P.bias])
        ...
        embeds, self.inverW_P.weight.data, self.inverW_P.bias.data = Mv2Samedevice([embeds, self.inverW_P.weight, self.inverW_P.bias])
...
class GTLayer(nn.Module):
    ...
    def forward(self, g, embeds):
        ...
        # 离散化处理：将原始索引映射到连续的索引
        # unique_nodes = t.unique(t.cat([rows, cols]))
        # node_mapping = {node.item(): i for i, node in enumerate(unique_nodes)}
        #
        # # 创建映射张量用于快速索引转换
        # max_node = max(rows.max(), cols.max()) + 1
        # mapping_tensor = t.full((max_node,), -1, dtype=t.long, device=rows.device)
        # for orig_idx, new_idx in node_mapping.items():
        #     mapping_tensor[orig_idx] = new_idx
        #
        # # 应用离散化映射
        # rows_discrete = mapping_tensor[rows]
        # cols_discrete = mapping_tensor[cols]
        # rows, cols = rows_discrete, cols_discrete
        # # 使用离散化后的索引
        # # discrete_embeds = embeds[unique_nodes]  # 只保留实际使用的节点嵌入
        # rowEmbeds = embeds[rows_discrete, :]
        # colEmbeds = embeds[cols_discrete, :]
