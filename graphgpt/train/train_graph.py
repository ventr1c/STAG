...
class ModelArguments:
    model_name_or_path: Optional[str] = field(default="/root/autodl-tmp/GraphGPT/checkpoints/stage_2/citeseer-checkpoint-400")
    # model_name_or_path: Optional[str] = field(default="/root/autodl-tmp/GraphGPT/GraphGPT-7B-mix-all")
    version: Optional[str] = field(default="v1")
    freeze_backbone: bool = field(default=False)
    tune_graph_mlp_adapter: bool = field(default=True)
    graph_tower: Optional[str] = field(default="clip_gt_arxiv")
    graph_select_layer: Optional[int] = field(default=-2)   # default to the last layer
    # pretrain_graph_mlp_adapter: Optional[str] = field(default='/root/autodl-tmp/GraphGPT/checkpoints/stage_1/checkpoint-1200/checkpoint-1200.bin')
    pretrain_graph_mlp_adapter: Optional[str] = field(
        default='/root/autodl-tmp/GraphGPT/checkpoints/stage_2/citeseer_graph_projector/checkpoint-400.bin')
    pretrain_graph_model_path: Optional[str] = field(default="/root/autodl-tmp/GraphGPT/backdoor_res/citeseer")
    use_graph_start_end: bool = field(default=True)


@dataclass
class DataArguments:
    data_path: str = field(default='/root/autodl-tmp/GraphCLIP/graphgpt_eval/citeseer_graphgpt_train.json',
    ...
    graph_data_path: Optional[str] = field(default='/root/autodl-tmp/GraphCLIP/processed_data/citeseer.pt')
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    output_dir: str = "/root/autodl-tmp/GraphGPT/checkpoints/stage_2"
    # device: str = "cuda:0"
    # distributed_state: str = "no"
...
def preprocess_v1(
    ...
    for i, source in enumerate(sources):
        ...
        for j, sentence in enumerate(source):
            ...
            start_marker = "with the following information:"
            end_marker = "Question:"
            text = sentence["value"]
            # 2. 找到开始标记的索引
            # text.find() 会返回子字符串第一次出现的索引
            ...
            if start_index != -1 and end_index != -1:
                # 第一部分：从开头 (0) 到 start_index (不包含开始标记)
                part1 = text[:start_index]

                # 第二部分：从 end_index (包含 "Question:") 到结尾
                part2 = text[end_index:]

                # 5. 拼接两部分
                # 我们用 .strip() 分别清理两部分末尾和开头的多余空白或换行
                # 然后在它们之间加一个换行符 \n 保持格式
                sentence["value"] = part1.strip() + "\n" + part2.strip()
...
class NonShufflingGraphChatTrainer(GraphChatTrainer):
    """
    一个自定义的 Trainer，它在训练期间不打乱数据，
    而是使用顺序采样器 (SequentialSampler)。
    """
    def _get_train_sampler(self, *args) -> Optional[torch.utils.data.Sampler]:
        """
        重写父类方法，强制返回一个 SequentialSampler。
        """
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        logging.warning("Loading data...")
        list_data_dict = json.load(open(data_path, "r"))
        # selected_list_data_dict = []
        # for i in range(len(list_data_dict)):
        #     graph_type = copy.deepcopy(list_data_dict[i]['id']).split('_')[0]
        #     if graph_type == "arxiv":
        #         selected_list_data_dict.append(list_data_dict[i])
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if task_type != 'LP': 
            if 'graph' in sources[0]:
                ...
                graph_node_rep = self.graph_data_all.x[graph_node_list] ##
                graph_node_y = self.graph_data_all.y[graph_node_list]
            ...
            if 'graph' in sources[0]:
                ...
                graph_type = copy.deepcopy(self.list_data_dict[i]['id']).split('_')[0]
                graph_node_rep_1 = self.graph_data_all.x[graph_node_list_1] ##
                ...
                graph_node_rep_2 = self.graph_data_all.x[graph_node_list_2] ##
        ...
        num_start = torch.where(data_dict["input_ids"] == 32001)[0].shape[0]
        ...
        if task_type != 'LP': 
            if 'graph' in self.list_data_dict[i]:
                ...
                data_dict['graph_data'] = Data(x = graph_node_rep, edge_index=graph_edge_index, target_node = torch.tensor([target_node]), y = graph_node_y)

            elif self.graph_cfg['is_graph']:
                # image does not exist in the data, but the model is multimodal
                node_feas = self.graph_cfg['graph_processor'].node_feas
                data_dict['graph_data'] = Data(x = torch.zeros(3, node_feas), edge_index=torch.zeros(2, 3), target_node = torch.tensor([0]), y = graph_node_y)
        else:
            if num_start != 2:
                raise ValueError("The number of <g_start> tokens is not equal to 2 in LP task.")
            if 'graph' in self.list_data_dict[i]:
                # data_dict['graph_node'] = graph_node_rep
                # data_dict['graph_edge'] = graph_edge_index
                # data_dict['target_node'] = target_node
                data_dict['graph_data'] = {
                    'graph_1': Data(x = graph_node_rep_1, edge_index=graph_edge_index_1, target_node = torch.tensor([target_node_1])),
                    'graph_2': Data(x = graph_node_rep_2, edge_index=graph_edge_index_2, target_node = torch.tensor([target_node_2]))
                    }

            elif self.graph_cfg['is_graph']:
                # image does not exist in the data, but the model is multimodal
                node_feas = self.graph_cfg['graph_processor'].node_feas
                data_dict['graph_data'] = Data(x = torch.zeros(3, node_feas), edge_index=torch.zeros(2, 3), target_node = torch.tensor([0]))
        return data_dict
...
def train():
    ...
    # 初始化 pretrain_graph_model_path
    model.config.pretrain_graph_model_path = model_args.pretrain_graph_model_path
    ...
    #                 args=training_args,
    #                 **data_module)
    trainer = NonShufflingGraphChatTrainer(model=model,
                    tokenizer=tokenizer,
                    args=training_args,
                    **data_module)
