...
class graph_transformer(nn.Module):
    ...
    def forward(self, g):
        ...
        x, self.W_P.weight.data, self.W_P.bias.data, self.W_pos.data = Mv2Samedevice([x, self.W_P.weight, self.W_P.bias, self.W_pos])
        z = self.W_P(x)
        if self.args.if_pos:
        ...
        embeds, self.inverW_P.weight.data, self.inverW_P.bias.data = Mv2Samedevice(
...
class GTLayer(nn.Module):
    ...
    def forward(self, g, embeds):
        ...
        rowEmbeds, self.qTrans.data, self.kTrans.data, self.vTrans.data = Mv2Samedevice(
        ...
        if self.args.att_norm:
            resEmbeds, self.norm.weight.data, self.norm.bias.data = Mv2Samedevice([resEmbeds, self.norm.weight, self.norm.bias])
